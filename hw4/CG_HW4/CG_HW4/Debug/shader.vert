...
vec4 vertLighting(){
	...
	vec4 vPos = vmn * av4position; // change vertex's position to eye space (camera space)

	//Normal transformation
	vec3 norN   = normalize(av3normal); // vertex normal (for 1 vertex)
	...
	for (int i = 0; i < 3; i++){
		...
		if (lightType[i] > 0.5f){ // if light on
			vec4 vv4specular_D = vec4(0, 0, 0, 1); // since w = 1, it's a position
			vec3 lightV;
			float attenuation;
			
			if (LightSource[i].position.w < 0.5){ // if this light is directional light
				...
				if (LightSource[i].spotCutoff != 180.0){ // angle 
					...
					vec3 rD = -lightV; // rD = vector: light position to vertex
					vec3 coneDir = normalize(LightSource[i].spotDirection.xyz);
			...
			float nxDir = max(0.0, dot(norN, lightV)); // cos (angle between light vector and vertex's normal)
			vec4 vv4diffuse_D = nxDir * Material.diffuse * LightSource[i].diffuse;
			
			// specular (reference to the ch4 p.21)
			if (nxDir != 0.0){ // angle != 90
				// get view vector
	...
	return vv4ambient_D + resLight;
}

void main() {
	if (lightType[3] < 0.5){
		vec4 res = vertLighting();
		vv4color = res;
	}
	else
		vv4color = vec4(av3normal, 0);

	// CG_HW2 done already
	// for model translation, scaling and rotation
